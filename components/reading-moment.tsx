...
export function ReadingMoment({onNote,data,save}:{onNote:(text:string)=>void;data:Data;save:(patch:Partial<Data>)=>void}){
 ...
 function changeBook(next:number){smoothDomUpdate(()=>setIndex(next));}
 ...
 <div className="reading-actions"><button className="round" aria-label="上一本书" onClick={()=>changeBook((index+books.length-1)%books.length)}><ChevronLeft size={18}/></button><small>{index+1} / {books.length}</small><button className="round" aria-label="下一本书" onClick={()=>changeBook((index+1)%books.length)}><ChevronRight size={18}/></button><button className="soft-button" onClick={()=>onNote(`读《${book.title}》 · ${book.author}\n${saved?.progress??''}\n${saved?.thought??''}\n`)}><Pencil size={15}/> 写点读后感</button></div>
