...
export function WeekReview({data,today}:{data:Data;today:string}){
 ...
 return <section className="card week-review"><h2>我们的最近七天</h2><div>{items.map(([label,value])=><p key={label}><strong>{value}</strong><small>{label}</small></p>)}</div><small>平凡的小事，也在一点点积攒。</small></section>;
}
