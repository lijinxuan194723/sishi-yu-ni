...
export function HeroGallery({season,children}:{season:keyof typeof seasonAlbums;children:ReactNode}){
 const [viewport,carousel]=useEmblaCarousel({loop:true,container:'.hero-track',slides:'.hero-slide',duration:25,watchDrag:(_api,event)=>!(event.target as HTMLElement)?.closest('button,input,a')});
 ...
 useEffect(()=>{carousel?.reInit();carousel?.scrollTo(0,true);},[carousel,season]);
