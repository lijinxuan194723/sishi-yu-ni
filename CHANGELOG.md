...
## v0.8（2026-09-10）
- 新增：夏彦人物知识库 V2，接入 412 条结构化人物记忆：人物基础 41、性格习惯 76、人物关系 38、能力兴趣 42、重要经历 155、日常细节 60。
- 新增：按当前聊天内容动态检索相关人物记忆，每轮只注入核心设定与少量相关条目，避免把整个知识库塞入 system prompt。
- 新增：保留 A / A2 / B / C 资料可信度层级，并记录条目来源编号、时期、剧情范围与是否涉及原作女主。
- 新增：默认隔离“换日线”、逸梦等平行/特殊世界线；只有当前话题明确询问时才允许召回。
- 修改：用户长期记忆与夏彦原作人物知识彻底分离；原作女主相关经历不会自动被写成当前用户的真实共同经历。
- 测试：新增 `check-luke-knowledge.mjs`，验证 412 条完整加载、分类数量、典型问答召回、AU 隔离与上下文大小；新增独立记忆依赖链 TypeScript 检查。
