...
export function saveBackupFile(name:string,text:string){if(isAndroid()){window.LukeAndroid!.saveBackup(name,text);return;}const url=URL.createObjectURL(new Blob([text],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download=name;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}
