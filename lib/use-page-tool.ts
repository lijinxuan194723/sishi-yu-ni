...
export function usePageTool(navigate:(page:string)=>void){
 ...
 try{Promise.resolve(context.registerTool({name:'open_companion_page',title:'打开夏日来信页面',description:'打开首页、悄悄话、他的此刻、日历或手记，不创建或更改记录。',inputSchema:{type:'object',properties:{page:{type:'string',enum:['home','chat','heart','calendar','notes']}},required:['page'],additionalProperties:false},annotations:{readOnlyHint:false},execute(input){const page=(input as {page?:unknown})?.page;if(typeof page!=='string'||!['home','chat','heart','calendar','notes'].includes(page))throw Error('未知页面');flushSync(()=>navigate(page));return {page};}},{signal:lifecycle.signal})).catch(()=>{});}catch{}
