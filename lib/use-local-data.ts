...
export function useLocalData(initial:Data){
 ...
 useEffect(()=>{try{let raw=localStorage.getItem(key);if(!localStorage.getItem('luke-name-default-v2')){if(raw){const value=parseData(raw);if(value.name==='我'){raw=JSON.stringify({...value,name:'冬清'});localStorage.setItem(key,raw);}}localStorage.setItem('luke-name-default-v2','1');}saved.current=raw;if(raw)setData(parseData(raw));setReady(true);}catch{setError('原有存档无法读取，暂不覆盖。请先导出原始存档。');setStatus('本地记忆未载入');}
